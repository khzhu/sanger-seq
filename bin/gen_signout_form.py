...
parser.add_argument('-i', '--input_path', type=str, dest='input_path', required=True,
                    help='Path to SangerSeq analysis results')
parser.add_argument('-o', '--output_path', type=str, dest='output_path', required=True,
...
def change_table_border_color(table, color):
    """Change the border color of a table in python-docx."""
...
def shade_table_header(table, color):
    """Shades the header row of a table in a docx document."""
...
def set_cell_margins(table, top=0, bottom=0):

    tc = table._element
    tblPr = tc.tblPr
    tblCellMar = OxmlElement('w:tblCellMar')
    kwargs = {"top":top, "bottom":bottom}
    for m in ["top","bottom"]:
...
def add_table_footnote():
    ...
    run = list_paragraph.add_run('\u2461' + 
        ' Read Length refers to the total number of base pairs (bp) sequenced from the forward or reverse sequences in a specimen.')
    font = run.font
    font.name = 'Calibri'
    font.size = Pt(10)
    ...
    list_paragraph.add_run('\n')
    run = list_paragraph.add_run('\u2464' + 
        ' Contig Length refers to the length of a contiguous Sanger sequence generated by assembling forward and reverse sequences in a specimen.')
    font = run.font
    font.name = 'Calibri'
    font.size = Pt(10)
    font.color.rgb = RGBColor(4, 118, 208)# Light blue color
    list_paragraph.add_run('\n')
    run = list_paragraph.add_run('\u2465' + 
        ' Signal Peak corresponds to a specific nucleotide base, with the peak height indicating the signal strength for that base.')
    font = run.font
    font.name = 'Calibri'
    font.size = Pt(10)
    font.color.rgb = RGBColor(4, 118, 208) # Light blue color
...
def add_table_headnote():
    ...
    list_paragraph.add_run('\n')
    run = list_paragraph.add_run('\u2462' + 
        ' Alignment score is based on the number of matches, mismatches, and gaps and indicates the sequence similarity between the query and subject.')
    font = run.font
    font.name = 'Calibri'
    font.size = Pt(10)
    font.color.rgb = RGBColor(4, 118, 208)# Light blue color
    list_paragraph.add_run('\n')
    run = list_paragraph.add_run('\u2464' + 
        ' Is an estimate of the Expected number of random alignments with a particular score or better that could be found by chance in each database search. E-values close to zero correspond to more significant alignments.')
    font = run.font
    font.name = 'Calibri'
    font.size = Pt(10)
    font.color.rgb = RGBColor(4, 118, 208) # Light blue color
...
images = convert_from_path(fwd_chromatogram, poppler_path="/opt/homebrew/Cellar/poppler/24.11.0/bin")
for image in images:    
...
images = convert_from_path(rev_chromatogram, poppler_path="/opt/homebrew/Cellar/poppler/24.11.0/bin")
for image in images:    
...
# Save the document
doc.save(os.path.join(args.output_path, args.sample, 'patient_signout_form.docx'))