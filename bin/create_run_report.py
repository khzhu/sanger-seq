...
def add_patient(qc_csv, fwd_chromatogram, rev_chromatogram, fwd_chromatogram_pdf, 
                     rev_chromatogram_pdf, contig_fasta, forward_fasta, reverse_fasta,
                     blast_tsv, blast_html, sample, pos_ctl, s_type):
    if sample.startswith(s_type) and sample.endswith("NFW"):
        neg_header = doc.add_heading('Negative Control', level=1)
        neg_header.alignment = WD_ALIGN_PARAGRAPH.CENTER
    elif sample.startswith(s_type) and sample.endswith(pos_ctl):
        pos_header = doc.add_heading('Positive Control', level=1)
        pos_header.alignment = WD_ALIGN_PARAGRAPH.CENTER
    elif sample.startswith(s_type):
        sample_header = doc.add_heading(sample, level=1)
        sample_header.alignment = WD_ALIGN_PARAGRAPH.CENTER
        doc.add_heading('Patient Demographic Information', level=2)
        ...
        table1.cell(2, 1).text = sample.split("-")[-1]
    ...
    if sample.startswith(s_type) and sample.endswith("NFW"):
        doc.add_heading('Negative Control Quality Metrics', level=2)
    elif sample.startswith(s_type) and sample.endswith(pos_ctl):
        doc.add_heading('Positive Control Quality Metrics', level=2)
    else:
        doc.add_heading('Sample Quality Metrics', level=2)
    ...
    # Add an image
    if sample.startswith(s_type) and sample.endswith(pos_ctl):
        doc.add_heading('Positive Control Electropherogram\u00B9', level=2)
    elif sample.startswith(s_type) and sample.endswith("NFW"):
        doc.add_heading('Negative Control Electropherogram\u00B9', level=2)
    else:
        doc.add_heading('Patient Electropherogram\u00B9', level=2)
   
    fwd_h3 = doc.add_heading('1. Forward Electropherogram', level=3)
    add_hyperlink(fwd_h3, fwd_chromatogram_pdf, '(View PDF)')

    images = convert_from_path(fwd_chromatogram, poppler_path="/opt/homebrew/opt/poppler/bin") #/usr/bin
    for image in images:    
        image.save(os.path.join(args.output_path, sample, 'fwd_chromatogram.png'), 'PNG')
    doc.add_picture(os.path.join(args.output_path, sample,'fwd_chromatogram.png'), width=Pt(500))
    rev_h3 = doc.add_heading('2. Reverse Electropherogram', level=3)

    add_hyperlink(rev_h3, rev_chromatogram_pdf, '(View PDF)')
    images = convert_from_path(rev_chromatogram, poppler_path="/opt/homebrew/opt/poppler/bin") #/usr/bin
    for image in images:    
        image.save(os.path.join(args.output_path, sample,'rev_chromatogram.png'), 'PNG')
    doc.add_picture(os.path.join(args.output_path, sample,'rev_chromatogram.png'), width=Pt(500))

    # add consensus seqeuence tables
    create_seq_table('Consensus Sequence\u00B9', contig_fasta)
    create_seq_table('1. Forward Sequence', forward_fasta)
    create_seq_table('2. Reverse Sequence', reverse_fasta)

    try:
        blast_tab = doc.add_heading('BLAST Analysis of the Consensus Sequence', level=2)
        ...
        pass
    if sample.startswith(s_type):
        if not sample.endswith(pos_ctl)and not sample.endswith("NFW"):
            doc.add_heading('Patient Signout', level=2)
...
def add_sample_to_page(subdir, pos_ctrl, s_type):
    sample = os.path.basename(subdir)
    qc_csv = os.path.join(subdir, "%s_qc_metrics.csv"%sample)
    fwd_chromatogram = os.path.join(subdir, "%s_F.chromatogram_100bases.pdf"%sample)
    rev_chromatogram = os.path.join(subdir, "%s_R.chromatogram_100bases.pdf"%sample)
    fwd_chromatogram_pdf = "/".join([args.share_point, "%s_F.chromatogram.pdf?csf=1&web=1"%sample])
    rev_chromatogram_pdf = "/".join([args.share_point, "%s_R.chromatogram.pdf?csf=1&web=1"%sample])
    contig_fasta = os.path.join(subdir, "%s_consensus_sequence.txt"%sample)
    forward_fasta = os.path.join(subdir, "%s_forward_sequence.txt"%sample)
    reverse_fasta = os.path.join(subdir, "%s_reverse_sequence.txt"%sample)
    blast_tsv = os.path.join(subdir, "blast", "%s_blast.tsv"%sample)
    blast_html = "/".join([args.share_point, "blast", "%s_blast_identity.html?csf=1&web=1"%sample])

    add_patient(qc_csv, fwd_chromatogram, rev_chromatogram, fwd_chromatogram_pdf, 
                     rev_chromatogram_pdf, contig_fasta, forward_fasta, reverse_fasta,
                     blast_tsv, blast_html, sample, pos_ctrl, s_type)
...
# Add a title
title = doc.add_heading(args.batch_id, level=1)
...
# add Positive Control to the page if exists
for subdir, dirs, files in os.walk(args.input_path):
    sample = os.path.basename(subdir)
    if sample.endswith(args.positive_control):
        add_sample_to_page(subdir, args.positive_control, args.sample_type)
        break

# add Negative Control to the page
for subdir, dirs, files in os.walk(args.input_path):
    sample = os.path.basename(subdir)
    if  sample.startswith(args.sample_type) and sample.endswith("NFW"):
        add_sample_to_page(subdir, args.positive_control, args.sample_type)
        break

# add patient samples to the page
for subdir, dirs, files in os.walk(args.input_path):
    sample = os.path.basename(subdir)
    if sample.startswith(args.sample_type):
        if sample.endswith("NFW") or \
            sample.endswith(args.positive_control): continue
        add_sample_to_page(subdir, args.positive_control, args.sample_type)

appendix = doc.add_heading("Appendix", level=1)
appendix.alignment = WD_ALIGN_PARAGRAPH.CENTER
doc.add_heading("Note on Interpretation and Reporting", level=2)
doc.add_heading("Electropherogram", level=3)
add_pherogram_note()
doc.add_heading("Consensus Sequence", level=3)
add_contig_note()
# add table note
doc.add_heading("Quality Control Metrics", level=3)
add_table_footnote()
doc.add_heading("BLAST Search Report", level=3)
add_table_headnote()
...
# Save the document
doc.save('./%s_%s_summary_report.docx'%(args.batch_id, args.sample_type))